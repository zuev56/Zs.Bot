...
namespace Zs.Bot.Model.Db
{
    public partial class ZsBotDbContext : DbContext
    {
        ...
        public virtual DbSet<DbQuery> SqlResults { get; set; }
        ...
        public static string GetStringQueryResult(string query)
        {
            ...
            var fromSqlRaw = ctx.SqlResults.FromSqlRaw($"{query} as Result").AsEnumerable();

            return fromSqlRaw.ToList()?[0]?.Result ?? "NULL";

            //ВЫГРУЗИТЬ ВСЕХ ПОЛЬЗОВАТЕЛЕЙ В JSON
            //И РЕАЛИЗОВАТЬ НОРМАЛЬНУЮ ПОДГРУЗКУ СООБЩЕНИЙ В БД, СОЗДАВАЯ ПОЛНУЮ ЭМИТАЦИЮ
        }
    }
}
